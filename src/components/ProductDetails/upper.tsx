// ProductDetails.tsx
import { Flame, ShoppingCart, Star,Box,PenBox , Store , Gift , Heart, Share2 } from 'lucide-react';
...
const ProductDetailsUpper = () => {
    ...
    const fetchProduct = async () => {
      try {
        const res = await axios.get(
          `https://salla.digital-vision-solutions.com/api/products/1${id}`
        );
        setProduct(res.data);
  ...
  if (!product) {
    return <div className='justify-center items-center flex mt-8'>Product not found</div>;
  }
  ...
  return (
    <div dir="rtl" className="font-regular mx-auto p-4 max-w-screen-xl">
        <div >
            <Link to="/"><span className='text-[#62d0b6] text-md'> الرئيسية /<Link to='/products'><span>كل المنتجات /</span></Link> <span className='text-md text-gray-600'>تفاصيل المنتج</span></span></Link>
        </div>
...
<div className="flex items-center gap-x-4 mt-4">
  ...
  <div className="flex justify-between items-center">
    ...
    >
      {isOpen ? <span className='relative w-8 h-8 rounded-full border-2 cursor-pointer flex items-center justify-center bg-[#62d0b6] text-white'>-</span>   : <span className='relative w-8 h-8 rounded-full border-2 cursor-pointer flex items-center justify-center'>+</span>}
    </button>
  </div>
...
<div className="border p-2 rounded flex justify-between items-center">
  ...
  <button className="relative w-8 h-8 rounded-full border-2 cursor-pointer flex items-center justify-center">+</button>
</div>
<div className="border p-2 rounded flex justify-between items-center">
  <div className="flex items-center gap-1 text-[#62d0b6]">
    <PenBox className="w-4 h-4" />
